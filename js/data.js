...
const PH_DATA = {
  ...
  saveToStorage() {
    try {
      localStorage.setItem('padowa_customers', JSON.stringify(this.customers));
      localStorage.setItem('padowa_invoices', JSON.stringify(this.invoices));
      localStorage.setItem('padowa_products', JSON.stringify(this.products));
      localStorage.setItem('padowa_next_invoice_seq', this.nextInvoiceSeq.toString());
    } catch (e) {
  ...
  loadFromStorage() {
    try {
      const customersStr = localStorage.getItem('padowa_customers');
      if (customersStr) {
        this.customers = JSON.parse(customersStr);
      }
      
      const invoicesStr = localStorage.getItem('padowa_invoices');
      if (invoicesStr) {
        this.invoices = JSON.parse(invoicesStr);
      }
      
      const productsStr = localStorage.getItem('padowa_products');
      if (productsStr) {
      ...
      if (seqStr) {
        this.nextInvoiceSeq = parseInt(seqStr) || this.nextInvoiceSeq;
      } else {
        // Derive from existing invoices if no seq stored
        if (this.invoices.length > 0) {
          const nums = this.invoices.map(inv => {
            const match = inv.number && inv.number.match(/(\d+)$/);
            return match ? parseInt(match[1]) : 0;
          });
          const maxSeq = Math.max(0, ...nums);
          if (maxSeq >= this.nextInvoiceSeq) this.nextInvoiceSeq = maxSeq + 1;
        }
      }
    } catch (e) {
